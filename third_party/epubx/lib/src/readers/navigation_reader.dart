...
class NavigationReader {
  String _tocDirectory = '';

  static Future<EpubNavigation> readNavigation(Archive epubArchive,
          String contentDirectoryPath, EpubPackage package) =>
      NavigationReader()
          ._readNavigation(epubArchive, contentDirectoryPath, package);

  Future<EpubNavigation> _readNavigation(Archive epubArchive,
    ...
    if (package.Version == EpubVersion.Epub2) {
      ...
      _tocDirectory = ZipPathUtils.getDirectoryPath(tocManifestItem.Href!);
      final tocFileEntryPath = ZipPathUtils.combine(
          contentDirectoryPath, ZipPathUtils.fileName(tocManifestItem.Href!));
      var tocFileEntry = epubArchive.files.cast<ArchiveFile?>().firstWhere(
          (ArchiveFile? file) => file!.name == tocFileEntryPath,
          orElse: () => null);
...
//      }
    ...
    navigationContentNode.attributes
      ...
      switch (navigationContentNodeAttribute.name.local.toLowerCase()) {
        case 'id':
          ...
          break;
        case 'src':
          result.Source = path.posix.join(_tocDirectory, attributeValue);
          break;
      }
    });
  ...
  String extractContentPath(String _tocFileEntryPath, String ref) {
