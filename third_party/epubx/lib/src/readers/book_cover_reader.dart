...
class BookCoverReader {
  static Future<images.Image?> readBookCover(EpubBookRef bookRef) async {
    ...
    final coverPath = ZipPathUtils.fileName(coverManifestItem.Href!);
    if (!bookRef.Content!.Images!.containsKey(coverPath)) {
      throw Exception(
          'Incorrect EPUB manifest: item with href = \"${coverManifestItem.Href}\" is missing.');
    }

    coverImageContentFileRef = bookRef.Content!.Images![coverPath];
