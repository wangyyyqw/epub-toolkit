...
void main() {
  ...
  group('TXT 转 EPUB', () {
    test('1. UTF-8+BOM 编码的 TXT 转换无乱码', () async {
      ...
      expect(rawText, isNot(startsWith('\ufeff')),
          reason: 'BOM 应被去除');
      expect(rawText, contains('国王山努亚'),
          reason: '中文内容应正确读取，无乱码');
      ...
      print('✅ UTF-8+BOM txt2epub 测试通过: ${chapters.length} 章, '
    ...
    test('3. GBK 编码的 TXT 转换检查', () async {
      // Dart 无法直接编码 GBK，用一个简单的 GBK 字节序列
      // "国王" 的 GBK 编码: B9 FA CD F5
      final gbkBytes = <int>[
        ...[0xB9, 0xFA, 0xCD, 0xF5], // 国王
        ...utf8.encode('\n\n'), // 换行
        ...utf8.encode('第一章 测试\n'),
        ...utf8.encode('这是测试内容\n'),
      ];
      ...
      final rawText = EncodingDetector.readFile(txtPath, encoding);
      print('GBK 解码结果 (前50字): ${rawText.substring(0, rawText.length > 50 ? 50 : rawText.length)}');

      // 当前实现中 GBK 解码是降级方案，中文会变成替换字符
      // 这个测试主要验证不会崩溃
  ...
  group('恐妻家 EPUB 操作', () {
    // 恐妻家 epub 路径
    final epubPath = '/Users/aaa/Documents/github/epub-gadget/恐妻家 - [日]伊坂幸太郎.epub';
    ...
    test('0. 读取恐妻家 EPUB 结构', () {
      ...
      for (final f in archive.files) {
        if (f.name.toLowerCase().endsWith('.opf')) {
          ...
          print('OPF 路径: ${f.name}');
          print('OPF 前200字: ${content.substring(0, content.length > 200 ? 200 : content.length)}');
          break;
        }
      }
      ...
      print('文件统计: XHTML=$xhtmlCount, CSS=$cssCount, Images=$imageCount, Fonts=$fontCount');
    });
    ...
    test('6. 名称加密', () async {
      ...
      for (var i = 0; i < bytes.length - 4; i++) {
        if (bytes[i] == 0x50 && bytes[i+1] == 0x4B && bytes[i+2] == 0x03 && bytes[i+3] == 0x04) lfhCount++;
        if (bytes[i] == 0x50 && bytes[i+1] == 0x4B && bytes[i+2] == 0x01 && bytes[i+3] == 0x02) cdCount++;
      }
    ...
    test('7. 名称解密', () async {
      ...
      if (!File(encOut).existsSync()) {
        await EncryptOperation.execute(epubPath: epubPath, outputPath: encOut);
      }
    ...
    test('8. 字体加密', () async {
      ...
      try {
        await EncryptFontOperation.execute(epubPath: epubPath, outputPath: out);
        expect(File(out).existsSync(), true);
        ...
        print('✅ 字体加密通过 (LFH=$lfhCount, CD=$cdCount, maxZeroRun=$maxZeroRun)');
      } catch (e) {
        print('⚠️ 字体加密异常: $e');
        rethrow;
      }
