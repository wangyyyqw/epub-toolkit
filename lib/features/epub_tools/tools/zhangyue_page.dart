...
/// 掌阅转多看页面
///
/// 将掌阅格式散落的 aside 脚注转为多看标准格式，
/// 自动注入 note.png 图标和 epub 命名空间。
class ZhangyuePage extends StatefulWidget {
  const ZhangyuePage({super.key});

  @override
  State<ZhangyuePage> createState() => _ZhangyuePageState();
}

class _ZhangyuePageState extends State<ZhangyuePage> {
  /// 输入 EPUB 文件路径
  String _epubPath = '';
  ...
  Future<void> _execute() async {
    ...
    _logController.append('PROGRESS: 开始执行「得到转多看」操作...');
  ...
  <a class="duokan-footnote" epub:type="noteref" href="#note1" id="note_ref1">
    <img class="epub-footnote" src="../Images/old-note.png"/>
  </a>
</sup>

<aside epub:type="footnote" id="note1">
  <ol class="duokan-footnote-content">
    <li class="duokan-footnote-item">脚注内容</li>
  </ol>
</aside>''',
          isCode: true,
        ),
        ToolHelpSection(
          title: '转换后的处理',
          content:
              '会移除正文中散落的 aside，把脚注内容集中追加到 </body> 前；同时把 zhangyue-footnote 或 epub-footnote 图片统一改为注入的 note.png。',
        ),
        ToolHelpSection(
          title: '追加到正文末尾的多看脚注',
    ...
    <li class="duokan-footnote-item" id="note1">
      <p><a href="#note_ref1">脚注内容</a></p>
        ...
        ToolHelpSection(
          title: '引用锚点要求',
          content:
              '脚注引用需要带 duokan-footnote class，并通过 href="#noteId" 指向对应 aside。id 可以在 href 前或后；缺失时会使用 noteId_ref 作为返回锚点。',
        ),
      ],
    );
  }
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                buildHelpInfoBar(
                  context,
                  text: '将得到格式散落的 aside 脚注转为多看标准格式。点击查看结构说明。',
