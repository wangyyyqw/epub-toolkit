...
/// 替换封面图片页面
///
/// 独立 StatefulWidget，将 EPUB 的封面图片替换为用户指定的新图片，
/// 并输出新的 EPUB 文件。
class ReplaceCoverPage extends StatefulWidget {
  const ReplaceCoverPage({super.key});

  @override
  State<ReplaceCoverPage> createState() => _ReplaceCoverPageState();
}

class _ReplaceCoverPageState extends State<ReplaceCoverPage> {
  // ==================== 页面配置 ====================
  ...
  Future<void> _executeOperation() async {
    if (_coverPath.isEmpty) {
      _showWarning('请先选择新封面图片');
      return;
    }
    _logAppend('PROGRESS: 新封面图片：$_coverPath');
