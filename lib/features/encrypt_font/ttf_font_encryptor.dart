...
class TtfFontEncryptor {
  ...
  /// 韩文 Hangul 码点范围，按 Wails/Python 版 `range(0xAC00, 0xD7AF)` 对齐。
  static const int _hangulStart = 0xAC00;
  static const int _hangulEndExclusive = 0xD7AF;
  ...
  static EncryptResult encrypt(Uint8List fontData, Set<int> characters) {
    try {
      ...
      // 1. 解析原始 cmap。加密时必须保留原始映射，否则任何未被
      // HTML 替换覆盖、但仍使用该字体的文本都会显示为缺字。
      final originalCmap = parser.parseBestCmap();
      final charToGlyph = <int, int>{
        for (final char in characters)
          if (originalCmap[char] != null) char: originalCmap[char]!,
      };
      ...
      final availableCodes = List.generate(
        _hangulEndExclusive - _hangulStart,
        (i) => _hangulStart + i,
      ).where((code) => !originalCmap.containsKey(code)).toList();
      ...
      for (final charCode in characters) {
        final glyphId = charToGlyph[charCode];
      ...
      // 3. 重建 cmap 表（原始映射 + 影子映射）
      final mergedCmap = <int, int>{...originalCmap, ...shadowToGlyph};
      final newCmapData = parser.buildEncryptedCmap(mergedCmap);
...
class _FontParser {
  ...
  Map<int, int> parseBestCmap() {
    ...
    if (format12Offset != null) {
      return _parseCmapFormat12(format12Offset);
    } else if (format4Offset != null) {
      return _parseCmapFormat4(format4Offset);
    }
  ...
  Map<int, int> _parseCmapFormat4(int offset) {
    ...
    for (var seg = 0; seg < segCount; seg++) {
      ...
      for (var char = startCode; char <= endCode && char <= 0xFFFF; char++) {
        int glyphId;
        if (idRangeOffset == 0) {
  ...
  Map<int, int> _parseCmapFormat12(int offset) {
    ...
    for (var group = 0; group < numGroups; group++) {
      ...
      for (var char = startCharCode; char <= endCharCode; char++) {
